...
INSERT INTO submissions (cat,q,a,fq,fa,difficulty,tags,company,status,source)
SELECT 'Performance','How do you improve Core Web Vitals (LCP and CLS) on a content page?','**LCP**: optimize and preload the hero image, serve modern formats (WebP/AVIF), and inline critical CSS to speed first paint.

**CLS**: reserve space with `width`/`height` or `aspect-ratio` on images, ads, and embeds; and preload web fonts, matching fallback metrics with `size-adjust`/`ascent-override` (or `font-display: optional`) so a late font swap doesn''t reflow the layout.','What''s the most common cause of a bad CLS score?','Images, ads, and embeds inserted without reserved dimensions — they push existing content down when they load. Fix by setting explicit `width`/`height` or an `aspect-ratio` so the browser reserves the space up front.','advanced',ARRAY['core-web-vitals','performance','lcp','cls']::text[],'Mawdoo3','approved','core'
...
INSERT INTO submissions (cat,q,a,fq,fa,difficulty,tags,company,status,source)
SELECT 'React','How do you display real-time weather data that updates every second efficiently?','**Decouple network cadence from render cadence.** Stream via WebSocket, but don''t `setState` on every message when they arrive faster than the screen paints: buffer incoming data and flush to state on a throttled interval or via `requestAnimationFrame` (~1 render/frame). Memoize charts/rows with `React.memo` so only changed series re-render, and keep heavy formatting out of render. At a genuine ~1 update/sec a plain `setState` per message is already fine — batching earns its keep only once updates get bursty, sub-second, or hit many series at once.','When does calling `setState` on every socket message actually cause jank?','Only when messages arrive faster than the browser can paint (bursty, sub-second, or many series updating together): each `setState` can trigger its own render and overwhelm the main thread. Throttling to ~1 render/frame caps the work regardless of message rate. At a steady one message per second there''s no problem — the technique matters as the rate climbs.','advanced',ARRAY['real-time','websockets','performance','react']::text[],'Arabia Weather','approved','core'
